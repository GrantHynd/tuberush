/**
 * AWS Lambda: daily connections puzzle generation for TubeRush.
 * Triggered by EventBridge Scheduler (cron).
 * Uses the Connections Generator Agent Skill for high-quality puzzle generation.
 */
...
interface Secrets {
  ...
  ANTHROPIC_CONNECTIONS_SKILL_ID?: string;
  ANTHROPIC_CONNECTIONS_SKILL_VERSION?: string;
...
async function callClaude(
  ...
  if (skill) {
    ...
    const systemWithSkill =
      `The "Connections Generator" Agent Skill is active. Use it for quality, then output ONLY JSON (no markdown).\n\n${system}`;
...
function validateConnections(
  ...
  for (const g of o.groups as Record<string, unknown>[]) {
