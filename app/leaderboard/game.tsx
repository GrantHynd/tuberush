...
export default function GameLeaderboardScreen() {
  ...
  useEffect(() => {
    capture("leaderboard_viewed", { game_type: gt });
  }, [gt]);
  ...
  const { top, userOutside } = useMemo(() => {
    ...
    const userInTop = user ? top.some((e) => e.userId === user.id) : true;
    const userOutside =
      !userInTop && user
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.loadingContainer}>
          <Text style={styles.emptyText}>No scores yet for this week.</Text>
        </View>
      ) : (
        <ScrollView
